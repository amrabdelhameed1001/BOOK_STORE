...
CREATE TABLE AUTHORS(
 ...
 CREATE TABLE BOOK (
  ...
  foreign key (Author) references AUTHORS(Author_name) on update cascade
  );
...
select count(cart_id)+1 from shopping_cart into ordernum;
IF New.in_stock < new.threshold   THEN  
insert into SHOPPING_CART values(ordernum,new.isbn,new.threshold - new.in_stock);
END IF;
END;
//
DELIMITER ; 
...
DELIMITER // 
CREATE TRIGGER  confirm_orders  before delete ON Shopping_cart
FOR EACH ROW
BEGIN
